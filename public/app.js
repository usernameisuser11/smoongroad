...
const state={
  ...
  lastFitScores:JSON.parse(storage.get('smoongroad_fit_scores','null')),
  careerFilter:'all',
  activeSemester:'3-2',
...
const careerOpportunities=[
  {id:'sw-hackathon',type:'contest',title:'SW·AI 해커톤과 개발 공모전',source:'링커리어 공모전',desc:'짧은 기간 안에 아이디어를 서비스로 구현하고 팀 개발 경험을 만드는 활동',tags:['AI·소프트웨어','데이터','공학'],majorIds:['cs','humanai','bigdata','fintech','aiot','game','smartprod'],traits:{logic:82,creative:78,people:50,practical:96},url:'https://linkareer.com/list/contest',roadmapType:'project'},
  {id:'it-intern',type:'intern',title:'IT·개발·데이터 인턴',source:'링커리어 인턴',desc:'프론트엔드·백엔드·AI·데이터 직무의 실무 흐름과 협업 방식을 경험하는 인턴',tags:['AI·소프트웨어','데이터','시스템'],majorIds:['cs','humanai','bigdata','fintech','aiot','game','smartprod'],traits:{logic:92,creative:58,people:46,practical:92},url:'https://linkareer.com/list/intern',roadmapType:'career'},
  {id:'open-source-club',type:'education',title:'개발 동아리·오픈소스 프로젝트',source:'링커리어 동아리',desc:'부담이 큰 인턴 전에 협업 기록과 포트폴리오를 차근차근 쌓는 활동',tags:['AI·소프트웨어','프로젝트','협업'],majorIds:['cs','humanai','bigdata','fintech','aiot','game'],traits:{logic:76,creative:70,people:68,practical:90},url:'https://linkareer.com/list/club',roadmapType:'project'},
  {id:'data-contest',type:'contest',title:'데이터 분석·정책 제안 공모전',source:'링커리어 공모전',desc:'공공·산업 데이터를 분석해 문제를 정의하고 근거 있는 해결안을 제시하는 공모전',tags:['데이터','분석','정책'],majorIds:['bigdata','fintech','economics','business','space','public'],traits:{logic:96,creative:58,people:42,practical:78},url:'https://linkareer.com/list/contest',roadmapType:'project'},
  {id:'planning-marketing',type:'activity',title:'서비스 기획·마케팅 대외활동',source:'링커리어 대외활동',desc:'시장과 사용자를 조사하고 콘텐츠·캠페인·서비스 개선안을 직접 기획하는 활동',tags:['비즈니스','콘텐츠','사람'],majorIds:['business','globalbiz','convergencebiz','economics','history','japan','fashion'],traits:{logic:66,creative:82,people:82,practical:78},url:'https://linkareer.com/list/activity',roadmapType:'activity'},
  {id:'business-intern',type:'intern',title:'기획·마케팅·금융 인턴',source:'링커리어 인턴',desc:'시장조사·사업기획·마케팅·금융 데이터 등 전공과 연결되는 실무형 인턴',tags:['비즈니스','경제·금융','데이터'],majorIds:['business','globalbiz','convergencebiz','economics','fintech','ip'],traits:{logic:80,creative:66,people:72,practical:86},url:'https://linkareer.com/list/intern',roadmapType:'career'},
  {id:'education-mentor',type:'activity',title:'교육봉사·학습 멘토링',source:'링커리어 대외활동',desc:'학습자를 직접 만나 설명·상담·수업 운영 경험을 쌓는 교육 관련 활동',tags:['교육','사람','소통'],majorIds:['koredu','engedu','education','mathedu','family','library'],traits:{logic:58,creative:62,people:98,practical:76},url:'https://linkareer.com/list/activity',roadmapType:'activity'},
  {id:'public-social',type:'activity',title:'공공정책·사회문제 해결 활동',source:'링커리어 대외활동',desc:'지역·복지·환경·공공 문제를 조사하고 시민 관점의 개선안을 만드는 활동',tags:['법·정책','사회','사람'],majorIds:['public','security','space','family','ip','library'],traits:{logic:82,creative:58,people:82,practical:72},url:'https://linkareer.com/list/activity',roadmapType:'activity'},
  {id:'content-contest',type:'contest',title:'영상·스토리·브랜딩 콘텐츠 공모전',source:'링커리어 공모전',desc:'기획부터 제작까지 완성해 전공 감각과 포트폴리오를 함께 보여주는 공모전',tags:['콘텐츠','예술','스토리'],majorIds:['animation','game','history','japan','fashion','finearts','livingarts','music','dance'],traits:{logic:42,creative:100,people:55,practical:88},url:'https://linkareer.com/list/contest',roadmapType:'project'},
  {id:'design-club',type:'education',title:'디자인·공연·창작 프로젝트 팀',source:'링커리어 동아리',desc:'전시·공연·영상·브랜드 결과물을 함께 제작하며 협업 포트폴리오를 쌓는 활동',tags:['예술','콘텐츠','프로젝트'],majorIds:['animation','game','fashion','finearts','livingarts','music','dance'],traits:{logic:38,creative:98,people:72,practical:91},url:'https://linkareer.com/list/club',roadmapType:'project'},
  {id:'bio-research',type:'intern',title:'바이오·식품·소재 연구 인턴',source:'링커리어 인턴',desc:'실험 설계·품질관리·생산·연구 지원 등 이공계 전공을 현장에 연결하는 인턴',tags:['생명','화학','공학'],majorIds:['biotech','chemenergy','materials','food'],traits:{logic:91,creative:48,people:38,practical:96},url:'https://linkareer.com/list/intern',roadmapType:'career'},
  {id:'engineering-education',type:'education',title:'공학 실무교육·현장 프로젝트',source:'링커리어 교육',desc:'전공 수업에서 다룬 도구를 실제 장비·데이터·프로젝트로 반복해보는 직무교육',tags:['공학','실험','현장'],majorIds:['electrical','aiot','smartprod','biotech','chemenergy','materials'],traits:{logic:88,creative:52,people:38,practical:98},url:'https://linkareer.com/list/education',roadmapType:'activity'},
  {id:'health-activity',type:'activity',title:'건강·스포츠·복지 프로그램 운영',source:'링커리어 대외활동',desc:'사람의 생활과 건강을 가까이에서 지원하며 현장 대응과 프로그램 운영을 경험하는 활동',tags:['건강','사람','현장'],majorIds:['sports','food','family'],traits:{logic:56,creative:52,people:94,practical:94},url:'https://linkareer.com/list/activity',roadmapType:'activity'},
  {id:'starter-training',type:'education',title:'관심 직무 기초교육·부트캠프',source:'링커리어 교육',desc:'아직 직무가 선명하지 않을 때 짧은 교육으로 실제 과제와 도구를 먼저 경험하는 선택지',tags:['기초','직무탐색','프로젝트'],majorIds:[],traits:{logic:68,creative:68,people:68,practical:82},url:'https://linkareer.com/list/education',roadmapType:'activity'}
];

const careerTypeLabels={activity:'대외활동',contest:'공모전',education:'교육·동아리',intern:'인턴·채용'};
...
function routeTo(route){
  ...
  if(route==='explore'){renderMajors();updateInterestContext()}
  if(route==='compare')renderCompare();
  if(route==='career')renderCareerRecommendations();
  if(route==='roadmap')renderRoadmap();
...
function careerContext(){
  const scores=state.lastFitScores&&typeof state.lastFitScores==='object'?state.lastFitScores:{logic:68,creative:68,people:68,practical:76};
  const profileMajor=majorById(state.profile?.major);
  const savedMajor=[...state.saved].map(majorById).find(Boolean);
  const comparedMajor=state.compare.map(majorById).find(Boolean);
  const interestMajor=(state.profile?.interests||[]).length?[...majors].sort((a,b)=>profileInterestScore(b)-profileInterestScore(a))[0]:null;
  const fitMajor=state.lastFitScores?[...majors].map(m=>({...m,match:fitMatch(m,scores)})).sort((a,b)=>b.match-a.match)[0]:null;
  return {scores,major:profileMajor||fitMajor||savedMajor||comparedMajor||interestMajor||null,usedFit:Boolean(state.lastFitScores)};
}

function careerOpportunityScore(opportunity,context){
  const traitDiff=Object.keys(context.scores).reduce((sum,key)=>sum+Math.abs((opportunity.traits[key]||65)-context.scores[key]),0)/4;
  const traitFit=Math.max(0,100-traitDiff);
  const majorBonus=context.major&&opportunity.majorIds.includes(context.major.id)?13:0;
  const interests=[...(context.major?.interests||[]),...(state.profile?.interests||[])];
  const tagBonus=opportunity.tags.filter(tag=>interests.some(interest=>interest.includes(tag)||tag.includes(interest))).length*3;
  const practicalBonus=opportunity.type==='intern'&&context.scores.practical>=80?3:0;
  return Math.max(60,Math.min(97,Math.round(48+traitFit*.34+majorBonus+tagBonus+practicalBonus)));
}

function careerRecommendationReason(opportunity,context){
  const topTrait=Object.entries(context.scores).sort((a,b)=>b[1]-a[1])[0][0];
  const traitText={logic:'분석적으로 문제를 푸는 성향',creative:'아이디어를 결과로 표현하는 성향',people:'사람과 소통하는 성향',practical:'직접 실행하며 배우는 성향'}[topTrait];
  if(context.major&&opportunity.majorIds.includes(context.major.id))return `${context.major.name}의 진로 방향과 ${traitText}이 함께 맞아요`;
  const overlap=opportunity.tags.find(tag=>(context.major?.interests||[]).some(interest=>interest.includes(tag)||tag.includes(interest)));
  return overlap?`${overlap} 관심 분야와 ${traitText}을 함께 살릴 수 있어요`:`${traitText}을 실제 경험으로 확인하기 좋은 활동이에요`;
}

function careerRoadmapKey(opportunity){
  const savedYear=Number(state.profile?.year);
  const targetYear=savedYear||((opportunity.type==='intern')?3:2);
  const currentSemester=new Date().getMonth()<6?'1':'2';
  return `${Math.min(4,Math.max(1,targetYear))}-${currentSemester}`;
}

function isCareerInRoadmap(id){
  const opportunity=careerOpportunities.find(item=>item.id===id);
  return Object.values(state.roadmap).some(items=>(items||[]).some(item=>
    item.recommendationId===id||(opportunity&&item.title===opportunity.title)
  ));
}

function addCareerToRoadmap(id){
  const opportunity=careerOpportunities.find(item=>item.id===id);if(!opportunity)return;
  if(isCareerInRoadmap(id)){toast('이미 로드맵에 추가한 추천입니다');return}
  const key=careerRoadmapKey(opportunity);
  state.roadmap[key]??=[];
  state.roadmap[key].push({title:opportunity.title,type:opportunity.roadmapType,memo:`수뭉로드 추천 · ${opportunity.source}에서 최신 일정 확인`,recommendationId:opportunity.id,sourceUrl:opportunity.url});
  persist();renderCareerRecommendations();trackEvent('career_add_roadmap',{id,key});toast(`${key.replace('-','학년 ')}학기에 추가했습니다`);
}

function renderCareerRecommendations(){
  const profile=$('#careerProfile'),grid=$('#careerGrid');if(!profile||!grid)return;
  const context=careerContext();
  const topTrait=Object.entries(context.scores).sort((a,b)=>b[1]-a[1])[0][0];
  const traitText={logic:'논리·분석',creative:'창작·표현',people:'사람·소통',practical:'실행·실습'}[topTrait];
  profile.innerHTML=`<div class="career-profile-copy"><span>MY RECOMMENDATION BASE</span><h2>${context.major?`${context.major.name}에서 이어지는 경험`:'관심 분야를 찾기 위한 첫 경험'}</h2><p>${context.usedFit?`궁합검사의 ${traitText} 성향과 ${context.major?.name||'관심 분야'} 정보를 함께 반영했습니다`:'궁합검사를 완료하면 학습 성향까지 반영해 추천 순서를 더 정교하게 조정합니다'}</p></div><div class="career-profile-score"><span>${context.usedFit?'궁합 반영':'기본 추천'}</span><strong>${traitText}</strong><button data-route="fit">${context.usedFit?'궁합 다시 검사':'궁합검사 시작'}</button></div>`;
  $$('[data-route]',profile).forEach(button=>button.onclick=()=>routeTo(button.dataset.route));

  const ranked=careerOpportunities.map(item=>({...item,match:careerOpportunityScore(item,context),reason:careerRecommendationReason(item,context)})).sort((a,b)=>b.match-a.match);
  const filtered=state.careerFilter==='all'?ranked:ranked.filter(item=>item.type===state.careerFilter);
  grid.innerHTML=filtered.slice(0,8).map((item,index)=>`<article class="career-card ${index===0&&state.careerFilter==='all'?'featured':''}"><div class="career-card-top"><span>${careerTypeLabels[item.type]}</span><strong>${item.match}%</strong></div><h3>${item.title}</h3><p>${item.desc}</p><div class="career-reason"><b>추천 이유</b><span>${item.reason}</span></div><div class="career-tags">${item.tags.map(tag=>`<i>${tag}</i>`).join('')}</div><div class="career-card-actions"><button data-career-roadmap="${item.id}" class="${isCareerInRoadmap(item.id)?'added':''}">${isCareerInRoadmap(item.id)?'로드맵에 추가됨':'로드맵에 추가'}</button><a href="${item.url}" target="_blank" rel="noopener">${item.source}에서 보기 ↗</a></div></article>`).join('');
  $$('[data-career-roadmap]',grid).forEach(button=>button.onclick=()=>addCareerToRoadmap(button.dataset.careerRoadmap));
  $$('#careerFilters [data-career-filter]').forEach(button=>{const active=button.dataset.careerFilter===state.careerFilter;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))});
}

function bindCareerFilters(){
  $$('#careerFilters [data-career-filter]').forEach(button=>button.onclick=()=>{state.careerFilter=button.dataset.careerFilter;renderCareerRecommendations();trackEvent('career_filter',{type:state.careerFilter})});
}
...
function init(){
  ...
  $('#majorStat').textContent=majors.length;$('#majorStat').closest('div').querySelector('span').textContent='서울캠퍼스 전공·학과';
  updateSavedCount();renderFilters();renderMajors();renderCompare();$('#currentYear').value=state.profile.year||'';renderRoadmap();renderCareerRecommendations();bindGlobal();bindV3();bindCareerFilters();renderAccountButton();updateInterestContext();
  const initial=location.hash.slice(1);if(['home','explore','compare','fit','career','roadmap'].includes(initial))routeTo(initial);else routeTo('home');
  if(!state.profile.onboardingDone)setTimeout(()=>openOnboarding(false),280);
}
...
function persist(){
  ...
  storage.set('smoongroad_auth',JSON.stringify(state.auth));
  storage.set('smoongroad_roadmap_title',state.roadmapTitle||'나의 상명 로드맵');
  storage.set('smoongroad_fit_scores',JSON.stringify(state.lastFitScores));
  scheduleCloudSync();
}
